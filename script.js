const questionsData = [
    ...
    },
    {
        ...
        coreConcept: "用字典或長度 26 的陣列計算字母出現次數，抵銷至零。",
        thoughtProcess: ["先檢查長度，不一樣絕對不是。", "建立字典 `[Character: Int]` 紀錄次數。", "走訪第一個字串次數 +1；第二個字串次數 -1。", "檢查字典所有 value 是不是都歸零。"],
        variations: "😈 變形考題：如果是 Unicode 字元怎麼辦？👉 只能乖乖用 Dictionary，不能用長度 26 的 Array 了。",
        complexity: { time: "O(n)", space: "O(1)" },
        script: "「雖然排序只要一行但需 O(n log n)，為了 O(n) 的效能，我會用 Dictionary 紀錄字元出現的淨次數。」",
