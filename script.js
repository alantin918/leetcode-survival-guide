const questionsData = [
    ...
    },
    {
        ...
        coreConcept: "利用 Hash Map 進行「字元頻率抵銷」，一邊累加、一邊扣除，若提早見底即代表不配對。",
        thoughtProcess: [
            "先決條件防禦 (Early Return)：若兩個字串長度不同，組成必不相同，直接回傳 False。",
            "建立一個雜湊表 `lookup = {}` 用來統計字元出現的頻率。",
            "第一階段 (建表)：遍歷字串 s。若字母不在字典，初始化為 1；若已存在，則次數 +1。",
            "第二階段 (抵銷)：遍歷字串 t。如果該字母不在 lookup 裡，或是次數已經被扣到 0，代表 t 出現了 s 沒有的字元或次數過多，直接回傳 False。",
            "每次成功配對，就將 lookup 中該字母的次數減 1 (抵銷)。",
            "如果能順利遍歷完 t 而沒有被擋下，代表兩邊字母次數完全抵銷，回傳 True！"
        ],
        variations: "😈 變形考題：如果是 Python，有沒有更短的寫法？👉 實務上可以直接用 `return collections.Counter(s) == collections.Counter(t)`，但在面試時，還是要展現手刻字典的能力！",
        complexity: { time: "O(n)", space: "O(1)" },
        script: "「為了達到 O(n) 的時間複雜度，我會使用 Hash Map 來紀錄頻率。先防禦性檢查長度，然後遍歷第一個字串建立頻率表。接著遍歷第二個字串進行抵銷，一旦發現未記錄的字元或頻率不足，就能提早結束，避免不必要的運算。」",
...
const kidExplanations = {
    ...
    217: "班上辦摸彩，確認有沒有人拿到重複的號碼。拿一個空袋子，把號碼一個一個放進去，每次放之前先看袋子裡有沒有一樣的，有就喊「重複了！」，沒有就放進去繼續。",
    242: "確認「listen」和「silent」是不是用同樣的字母拼的。把第一個字的每個字母計數記在格子裡（l=1, i=1...），再用第二個字把格子的數量一個一個扣掉，最後全部歸零就是「一樣」！",
    49:  "一堆英文字，把「用同樣字母組成的」分成一組。把每個字的字母排成 ABC 順序，排列一樣的就是同一組夥伴！例如「eat」→「aet」、「tea」→「aet」，所以它們是一組！",
    125: "「level」正著念倒著念一樣，這叫回文！先把不是字母數字的符號去掉，然後左右各派一個人往中間走，遇到不一樣就喊「不是回文！」，兩人相遇就是回文！",
    167: "跟找兩個加起來等於目標一樣，但這次積木已經從小排到大了！左右各站一個人，加起來太大就右邊的人往左走一步，太小就左邊的人往右走一步，超有效率！",
    15:  "找三個數字加起來等於零。先把數字從小排到大，每次固定一個「隊長」，剩下的兩個人用左右夾的方式找配對，找到後繼續換下一個隊長。",
    11:  "有一排高矮不同的牆，選兩面牆中間裝水，哪兩面裝最多？裝的水量是「距離 × 矮牆的高度」。從最左和最右開始，每次把矮的那面往中間換，看有沒有更多水的組合！",
    121: "你有一張過去每天股票價格的記錄，找哪天買、哪天賣賺最多。記住你看過「最便宜的那天」，每天算一下「如果今天賣能賺多少」，記住最大的利潤就行了！",
    3:   "在「abcabcbb」這串字母裡，找最長的不重複字母串。用橡皮圈框住一段字母，右邊不斷往右延伸，遇到重複的字母就把左邊往右縮，記住橡皮圈最大時有幾個字母。",
    424: "有一串字母，可以改其中 k 個，讓最長的「全是同一個字母的連續一段」盡量長。只要視窗裡「需要改的數量 ≤ k」就合法，一直把視窗撐大！",
    206: "一列手牽手向右走的小朋友，要讓他們反過來向左走。用三個人幫忙：記住「前一個」、「現在這個」、「下一個」，讓現在的小朋友轉身牽前一個，然後三人一起往右移。",
    21:  "兩列已排好順序的學生，要合成一列。從兩列各出最前面的比大小，小的先進新隊伍，直到有一列空了，把剩下那列全接到後面。",
    141: "一列手牽手的小朋友，有沒有人偷偷牽到前面去形成圈圈？讓烏龜每次走一步，兔子每次走兩步，如果有圈圈，兔子一定會從後面追上烏龜！",
    104: "一棵大樹有很多叉路，找最深的路有幾層。問左邊的枝子「你最深幾層？」，再問右邊的枝子「你最深幾層？」，取比較深的那個，再加上自己這一層就是答案！",
    226: "把一棵樹照鏡子！每個分叉點都把左右兩邊互換，然後叫左右兩邊的小樹也各自照鏡子，從根一路做到葉子就完成了。",
    100: "兩棵樹長得一不一樣？同時在兩棵樹上走，每個位置都比較形狀和數字，只要有一個地方不同就回答「不一樣」，全部通過才回答「一樣」。",
    20:  "括號要配對，像套娃一樣。拿一個碗，看到左括號就放進碗裡，看到右括號就從碗裡拿出最後放進去的，確認他們是一對。最後碗必須是空的才算合法！",
    155: "一個特殊的盒子，除了可以放和拿東西，還要隨時告訴你「盒子裡目前最小的數字是多少」。秘訣是放每個數字的時候，順便把「到目前為止最小的數字」也記在旁邊！",
    704: "猜數字遊戲！我想 1 到 100 之間的一個數，你來猜。聰明的猜法是每次猜中間的數：猜 50，對方說「太大」，就猜 25，每次砍掉一半的可能，最多猜 7 次就能找到！"
};
